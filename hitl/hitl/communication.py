...
class Communication(Node):
    ...
    def gt_acc_callback(self, msg: Twist):
        self.acceleration_xy = self.rotate_to_body_frame(msg.linear)
        ...
        if self.imu_updated and self.gt_pose_updated and self.gt_vel_updated and self.gt_acc_updated:
            ...
            self.get_logger().info(f"Velocity (Body Frame): {self.velocity_xy}")
            self.get_logger().info(f"Acceleration (Body Frame): {self.acceleration_xy}")
            self.reset_flags()
